...
const findElementInPosition = ([x, ...xs], position) => {
  if (position > 0) return findElementInPosition(xs, position - 1);
  
  return x;
};

const replaceElementInPosition = ([x, ...xs], position, replacement) => {
  if (position > 0) return [x].concat(replaceElementInPosition(xs, position - 1, replacement));
  
  return [replacement].concat(xs);
};

const replaceCharInPosition = ([x, ...xs], position, replacement) => {
  if (position > 0) return x + replaceCharInPosition(xs, position - 1, replacement);
...
// Piece movement functions (final result)
...
const recordMove = (moveHistory, aggregate) => {
  return `${moveHistory} ${aggregate}`;
};

const recordPosition = (positionHistory, aggregate) => {
  return `${positionHistory} ${aggregate}`;
}

const getTeam = ([x, _]) => {
  if (x !== undefined) {
    if (x.toUpperCase() === 'W') return 1;
